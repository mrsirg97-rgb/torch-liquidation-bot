 ...
 *
 * Simulates the lending-monitor's functionality using solana-agent-kit:
...
const main = async () => {
  ...
  console.log('LENDING AGENT TEST — Surfpool Mainnet Fork')
  console.log('='.repeat(60))
  ...
  for (const buyer of buyers) {
    ...
    try {
      ...
      if (buyCount % 10 === 0) {
        const detail = await torchGetToken(agent, mint)
        log(
  ...
  // ------------------------------------------------------------------
  // 4. Get Lending Info (via agent kit)
  // ------------------------------------------------------------------
  log('\n[4] Get Lending Info')
  try {
    ...
    ok(
      'getLendingInfo',
      `rate=${bpsToPercent(lending.interest_rate_bps)} | maxLTV=${bpsToPercent(lending.max_ltv_bps)} | liqThreshold=${bpsToPercent(lending.liquidation_threshold_bps)} | avail=${(lending.treasury_sol_available / LAMPORTS_PER_SOL).toFixed(2)} SOL | loans=${lending.active_loans ?? 'unknown'}`,
    )
  } catch (e: any) {
  ...
  // ------------------------------------------------------------------
  // 6. Watch Loan Position / Health (via agent kit)
  // ------------------------------------------------------------------
  ...
  // ------------------------------------------------------------------
  // 7. Confirm Borrow via SAID (via agent kit)
  // ------------------------------------------------------------------
  log('\n[7] Confirm Borrow (SAID)')
  if (borrowSig) {
    try {
      // torchConfirm uses agent.wallet internally for SAID verification,
      // so we need a borrower-scoped agent since the borrower signed this tx
      const borrowerKW = new KeypairWallet(borrowerWallet, RPC_URL)
      const borrowerAgent = new SolanaAgentKit(borrowerKW, RPC_URL, {})
      const result = await torchConfirm(borrowerAgent, borrowSig)
  ...
  // ------------------------------------------------------------------
  // 9. Verify Position Closed (via agent kit)
  // ------------------------------------------------------------------
